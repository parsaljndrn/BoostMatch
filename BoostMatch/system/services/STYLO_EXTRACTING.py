...
def extract_regex_features(text: str) -> Dict[str, float]:
    ...
    return {
        ...
        "regex_uppercase_ratio":   sum(c.isupper() for c in text) / chars,
        "regex_digit_ratio":       sum(c.isdigit() for c in text) / chars,
        "regex_punct_ratio":       len(re.findall(r'[.,!?;:"\'\-]', text)) / chars,
