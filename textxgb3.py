...
data1["merged1"] = data1["title"].astype(str).str.strip() + " " + data1["text"].astype(str).str.strip()
data1["merged1"] = data1["merged1"].fillna("").astype(str)
...
data2["merged"] = data2["title"].astype(str) + " " + data2["text"].astype(str)
data2 = data2[data2["merged"].str.strip() != ""]
data2["merged"] = data2["merged"].fillna("").astype(str)
...
vectorizer = TfidfVectorizer(max_features=20000)  # pick top 10000 words as features
X_all = vectorizer.fit_transform(pd.concat([data1["merged1"], data2["merged"]]))

# Split each dataset separately
X1 = vectorizer.fit_transform(data1["merged1"])
y1 = data1["label"]
